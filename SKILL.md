...
description: 'Write in Simplified English: lead with the next action, number multi-step work, restate state across turns, suppress tangents, make wins visible. Invoke with /simplify-to-english; stays on until "stop simplify-to-english".'
...
Write so there is only one possible meaning. Output is short, active, and consistent.
...
1. Ambiguity causes errors. A tired reader must not have to guess.
2. The reader may read in a second language or through translation. Plain forms survive translation.
3. Consistency teaches the pattern once. The same word and the same structure every time keep load low.
